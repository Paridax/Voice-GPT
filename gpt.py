...
class ChatGPT:
    def __init__(self, token, conversation_id = None, debug = False, headless = True):
        if token is None:
    ...
    def get_response(self, message):
        ...
        while self.is_generating():
            pass
        response = self.last_message().get("text")
        return response

    def transcribe_response(self, message, start=""):
        """
        ...
        while self.is_generating():
            sleep(0.1)
            newMessage = self.last_message().get("text")
            newChars = newMessage[len(message):]
            print(newChars, end="")
            message += newChars
        print("")
        response = self.last_message().get("text")
